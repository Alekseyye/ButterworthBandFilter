...
# определение требуемой разрядной сетки вычислителя
bit_depth = depth * (N + 1) # N + 1 - число битов для представления коэффициентов фильтра, а соответственно и сигнала через него прошедшего
print("разрядность сетки:", bit_depth)
